...
def deterministic_event_id(row: dict[str, Any]) -> str:
    canonical = {
        ...
        "platform": str(row.get("platform", "tiktok")),
        "label": str(row.get("label", "NEUTRAL")),
        "ts_bucket": int(float(row.get("ts", time.time()))),
    }
...
class ReplayStore:
    ...
    def ingest(self, rows: list[dict]) -> int:
        """Bulk-insert normalized replay rows."""
        if not rows:
            return 0
        ...
        with self._lock:
            try:
                ...
                _log.warning(
                    "replay_store_ingest_failed error=%s",
                    exc,
                )
                return 0
    ...
    def recent(self, limit: int = 50) -> list[dict]:
        return self._query(
            "SELECT * FROM sim_events ORDER BY ts DESC LIMIT ?",
            [limit],
        )

    def row_count(self) -> int:
        rows = self._query(
            "SELECT COUNT(*) AS n FROM sim_events",
            [],
        )
    ...
    def _query(self, sql: str, params: list) -> list[dict]:
        ...
        with self._lock:
            try:
                ...
                _log.warning(
                    "replay_store_query_failed sql=%r error=%s",
                    sql[:60],
                    exc,
                )
                return []
